...
# for mu1 in mu1s:
    ...
    #Filename = demand_type + '_mu_' + str(mu) +  '_mu1_' + str(mu1) +'_sd_' + str(int(sd))
    Filename = demand_type + '_mu_' + str(mu) +  '_mu1_' + str(200) +'_sd_' + str(int(sd))
    ...
    axes[1].set_ylim(0, 3) #1 - 5*np.std(ID_n), 1 + 5*np.std(ID_n)
...
prof_na = pd.DataFrame(prof_na)
prof_df = pd.DataFrame(prof_df)
prof_id = pd.DataFrame(prof_id)
